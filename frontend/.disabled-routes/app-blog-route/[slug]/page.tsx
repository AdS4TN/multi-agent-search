...
export default async function BlogPost({ params }: PageProps) {
  ...
  const title = asString(page.data.title, "Untitled");
  const description = asString(page.data.description);
  const author = asString(page.data.author);
  const thumbnail = asString(page.data.thumbnail);
  const date = new Date(asString(page.data.date));
        ...
        <aside className="hidden lg:block w-[350px] flex-shrink-0 p-6 lg:p-10 bg-muted/60 dark:bg-muted/20">
          <div className="sticky top-20 space-y-8">
            {author && isValidAuthor(author) && (
              <AuthorCard author={getAuthor(author)} />
            )}
